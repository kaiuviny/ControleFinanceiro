...
+ (SELECT CASE WHEN SUM(valor) > 0 THEN SUM(valor) ELSE 0 END				 FROM despesas_variaveis WHERE codigo_grupo_usuarios = 1 AND data_utilizada <= CURRENT_DATE() AND mes_id = MONTH(CURRENT_DATE()) AND ano = YEAR(CURDATE()) GROUP BY codigo_grupo_usuarios) AS DEPESAS_TOTAIS;
